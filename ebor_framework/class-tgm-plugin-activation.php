...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
        ...
        class TGM_Plugin_Activation {
                ...
                public function __construct() {
                        ...
                        $this->strings = array(
                                'page_title'                      => __( 'Install Required Plugins', 'marble' ),
                                'menu_title'                      => __( 'Install Plugins', 'marble' ),
                                'installing'                      => __( 'Installing Plugin: %s', 'marble' ),
                                'oops'                            => __( 'Something went wrong.', 'marble' ),
                                ...
                                'return'                          => __( 'Return to Required Plugins Installer', 'marble' ),
                                'plugin_activated'                => __( 'Plugin activated successfully.', 'marble' ),
                                'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'marble' ),
                        );
                ...
                public function install_plugins_page() {
                        ...
                        <div class="tgmpa wrap">
                                ...
                                <?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), 'marble' ); ?>
                        ...
                        if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
                                ...
                                if ( $this->is_automatic ) {
                                        ...
                                        if ( is_wp_error( $activate ) ) {
                                                ...
                                                echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'marble' ) . '</a></p>';
                                ...
                                foreach ( $this->plugins as $plugin ) {
                                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                                echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( $this->strings['return'], 'marble' ) . '</a></p>';
                                                $complete[] = $plugin;
                                                break;
                                        }
                                        /** Nothing to store */
                                ...
                                if ( empty( $complete ) ) {
                                        echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'marble' ) . '">' . __( 'Return to the Dashboard', 'marble' ) . '</a>' ) . '</p>';
                        ...
                        elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
                                ...
                                if ( is_wp_error( $activate ) ) {
                                        ...
                                        if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
                                                $msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'marble' ), '<strong>' . $plugin['name'] . '</strong>' );
                        ...
                        if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
                                ...
                                if ( ! empty( $message ) ) {
                                        ...
                                        foreach ( $message as $type => $plugin_groups ) {
                                                ...
                                                $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'marble' ), $imploded, $count ) . '</p>'; // All messages now stored
                                        }
 
                                        /** Setup variables to determine if action links are needed */
                                        $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'marble' ) . '</a>' : '';
                                        $show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'marble' ) . '</a>'  : '';
                                        ...
                                        $action_links = apply_filters(
                                                ...
                                                array(
                                                        ...
                                                        'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'marble' ) . '</a>',
                                                )
                                        );
